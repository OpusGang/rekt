...
def rekt(src, fix, left=0, right=0, top=0, bottom=0):
	if src.format == vs.YUV420P8 or vs.YUV420P10 or vs.YUV420P12 or vs.YUV420P16 or vs.YUV420P32 or vs.YUV422P8 or vs.YUV422P10 or vs.YUV422P12 or vs.YUV422P16 or vs.YUV422P32:
	    if src.width-left-right % 4 != 0 or src.height-top-bottom % 4 != 0:
			raise TypeError("rekt_fast: fix height and width must be mod 4.")
	else:
...
def black_detect(clip, thresh=None, fix=False, left=0, right=0, top=0, bottom=0):
	if thresh == None:
		if clip.format.bits_per_sample == 8:
			...
			raise TypeError("dirt: thresh argument required for bit depths outside 8 and 16.")
	if fix == False:
